...
blk_init_queue_node(request_fn_proc *rfn, spinlock_t *lock, int node_id)
{
	...
	if (!lock) {
		...
		OSA_REGISTER_SPINLOCK(&q->__queue_lock, "request_queue_t->__queue_lock", 29);
		lock = &q->__queue_lock;
	}
...
int __init blk_dev_init(void)
{
	...
	/* DEP: Register the congestion waitqueue locks here, for lack
	 * of a better place
	 */
	OSA_REGISTER_SPINLOCK(&congestion_wqh[0].lock, "wait_queue_head_t->lock", 23);
	OSA_REGISTER_SPINLOCK(&congestion_wqh[1].lock, "wait_queue_head_t->lock", 23);
