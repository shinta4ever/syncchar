...
static int ext3_fill_super (struct super_block *sb, void *data, int silent)
{
	...
	OSA_REGISTER_SPINLOCK(&sbi->s_next_gen_lock, "ext3_sb_info->s_next_gen_lock", 28);
	/* per fileystem reservation list head & lock */
	spin_lock_init(&sbi->s_rsv_window_lock);
	OSA_REGISTER_SPINLOCK(&sbi->s_rsv_window_lock, "ext3_sb_info->s_rsv_window_lock", 30);
