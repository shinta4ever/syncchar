...
static void tty_buffer_init(struct tty_struct *tty)
{
	...
	OSA_REGISTER_SPINLOCK(&tty->buf.lock, "tty->buf.lock", 13);
	tty->buf.head = NULL;
	tty->buf.tail = NULL;
	tty->buf.free = NULL;
}
...
static void initialize_tty_struct(struct tty_struct *tty)
{
	...
	OSA_REGISTER_SPINLOCK(&tty->read_lock, "tty->read_lock", 14);
...
void __init console_init(void)
{
	...
	// DEP: Just put this here, for lack of a better place
	OSA_REGISTER_SPINLOCK(&tty_ldisc_wait.lock, "wait_queue_head_t->lock", 23);
...
{
	OSA_REGISTER_SPINLOCK(&tty_sem.wait.lock, "wait_queue_head_t->lock", 23);
...
#ifdef CONFIG_UNIX98_PTYS
	OSA_REGISTER_SPINLOCK(&allocated_ptys_lock.wait.lock, "wait_queue_head_t->lock", 23);
