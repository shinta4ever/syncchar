...
static void kmem_list3_init(struct kmem_list3 *parent)
{
	...
	OSA_REGISTER_SPINLOCK(&parent->list_lock, "kmem_list3->list_lock", 22);
					    ...
					    int batchcount)
{
	...
	if (nc) {
		...
		OSA_REGISTER_SPINLOCK(&nc->lock, "array_cache->lock", 17);
	}
	return nc;
}
...
static void init_list(struct kmem_cache *cachep, struct kmem_list3 *list, int nodeid)
{
	...
	OSA_REGISTER_SPINLOCK(&ptr->list_lock, "kmem_list3->list_lock", 22);
...
void __init kmem_cache_init(void)
{
	...
	/* 
	 * DEP: register the cache_cache lock
	 */
	OSA_REGISTER_SPINLOCK(&cache_cache.spinlock, "kmem_cache->spinlock", 20);
	...
	void (*dtor)(void*, struct kmem_cache *, unsigned long))
{
	...
	OSA_REGISTER_SPINLOCK(&cachep->spinlock, "kmem_cache->spinlock", 20);
